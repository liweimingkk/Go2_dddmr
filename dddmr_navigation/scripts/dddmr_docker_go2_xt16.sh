...
Environment:
  ...
  GO2_DDS_PRIMARY_ADDRESS=<auto IPv4 address of GO2_NET_IFACE>
  GO2_DDS_ROBOT_TOPIC_PATTERNS=<default "rt/utlidar/* rt/uslam/*">
...
docker_base_args() {
  ...
  if [[ -n "${GO2_DDS_PRIMARY_ADDRESS_VALUE}" ]]; then
    args+=(--env "GO2_DDS_PRIMARY_ADDRESS=${GO2_DDS_PRIMARY_ADDRESS_VALUE}")
  fi
  if [[ -n "${GO2_DDS_ROBOT_TOPIC_PATTERNS_VALUE}" ]]; then
    args+=(--env "GO2_DDS_ROBOT_TOPIC_PATTERNS=${GO2_DDS_ROBOT_TOPIC_PATTERNS_VALUE}")
  fi
