...
the three raw sensor inputs used by that mapping run:
...
The resulting bag is a diagnostic input capture, not a self-contained,
deterministic replay artifact: static TF, measured time offsets, launch
parameters, and the build version are not embedded in it.
...
run_live_readiness_check() {
  ...
  docker run --rm \
    ...
    -e "RECORD_XT16_TOPIC=${RECORD_XT16_TOPIC}" \
...
set -u
python3 /root/dddmr_navigation/scripts/go2_xt16_lidar_preflight.py \
  --topic "${RECORD_XT16_TOPIC}" \
  --samples 3 \
  --timeout 15 \
  --qos-reliability best_effort \
  --expect-width 32000 \
  --expect-points-per-ring 2000
...
main() {
  ...
  export XT16_TOPIC="${RECORD_XT16_TOPIC}"
  export MOUTH_CLOUD_TOPIC="${RECORD_MOUTH_CLOUD_TOPIC}"
  export ODOM_TOPIC="${RECORD_ODOM_TOPIC}"
