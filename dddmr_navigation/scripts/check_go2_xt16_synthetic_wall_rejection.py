...
def main() -> int:
    ...
    ground_pub = node.create_publisher(PointCloud2, "map1/mapground", qos)
    ...
    try:
        ...
        # Keep the control goal on the 0.4 m synthetic ground grid.  A midpoint
        # such as y=1.0 is 0.2 m from either neighboring row and is rejected by
        # the production config's 0.15 m endpoint-projection limit before A*
        # can exercise the wall logic under test.
