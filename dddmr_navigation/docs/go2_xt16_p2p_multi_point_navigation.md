...
`EXECUTE <mission_id>`。`/mcl_pose` 超过 1.25 秒未更新时，命令门禁仍会立即
输出零速；如果定位状态和健康输入仍然在线，任务会在现有 6 秒定位恢复窗口内
保持当前导航点，等待一次新的 MCL 修正。定位进入 `LOST`、输入失联、恢复窗口
耗尽、P2P 失败、单点超时、停留期间移动或总监控超时都会取消任务、禁用 P2P
目标并执行现有停车清理。
