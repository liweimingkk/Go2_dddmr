...
如果“人工遥控”指终端键盘而非原厂遥控器，可在第三个宿主机终端使用
`scripts/go2_wasd_teleop.py`。它默认只预览按键，不创建 ROS publisher；真实遥控
必须按 `go2_xt16_sport_live_runbook.md` 完成 ROS/Unitree 环境、现场监护、急停路径
和确认短语检查后才使用 `--live`。录制期间仍须保持 recorded-route 控制器
`DISABLED`，且不得同时运行其他 Sport 速度控制器。先退出键盘遥控并确认
`StopMove`，再在录制终端按 `Ctrl-C` 保存路线。
...
路线和地图属于运行工件，不随本次代码移植提交。先用上一节录制出自己的路线，
再显式指定路线 ID、JSON 和对应地图目录。下面以 `park_route_a` 为例：

```bash
cd /home/kkkkkkq/new22/new2/dddmr_navigation

ROUTE_ID=park_route_a
REPO_ROOT=/home/kkkkkkq/new22/new2
GO2_RECORDED_ROUTE_EXPECTED_ID="${ROUTE_ID}" \
ROUTE_FILE="${REPO_ROOT}/bags/routes/${ROUTE_ID}.json" \
ROUTE_MAP_DIR="${REPO_ROOT}/bags/go2_xt16_mouth_mapping_20260714_153136_map_2026_07_14_07_31_36" \
...
可用 `route_tool.py inspect` 查看所选路线的首点；控制器虽然允许
`0.60 m / 0.35 m` 的起点包络，第一次测试仍应尽量贴近录制起点。确保整条短路线
...
拓扑、速度/决策 publisher 唯一性检查。宿主机 UDP 接收缓冲不足 `16 MiB` 时会
在启动 ROS 前失败；本地
`current_observation` 还必须在提示输入前和真正 enable 前分别连续通过 `20 s`
新鲜度检查。
...
真实适配器接通后，路线控制器仍为 `DISABLED`，此时只会发送 `StopMove`。
操作员再次核对现场后必须在同一终端输入：
