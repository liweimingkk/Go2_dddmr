...
size_t AssignTask(const std::vector<std::shared_ptr<std::vector<int>>>& index_all_thread, int thread_num, std::vector<std::vector<std::pair<int, int>>>* tasks){
    ...
    if (thread_num <= 0 || sampled_data.empty()) {
        return total;
    }
    // 确定每个进程分配voxel索引的上下界
    std::sort(sampled_data.begin(), sampled_data.end());
    const size_t sample_count = sampled_data.size();
    for(int i=0; i<thread_num; ++i){
        size_t start = (sample_count * static_cast<size_t>(i)) / thread_num;
        size_t end = (sample_count * static_cast<size_t>(i + 1)) / thread_num;
        auto& cur_task = (*tasks)[i];
        cur_task.resize(index_all_thread.size());
        if (start >= sample_count) {
            for(size_t j=0; j<cur_task.size(); ++j){
                cur_task[j].first = int(index_all_thread[j]->size());
                cur_task[j].second = int(index_all_thread[j]->size());
            }
            continue;
        }
        int pivot_start = sampled_data[start];
        const bool is_last_task = (i + 1 >= thread_num) || (end >= sample_count);
        int pivot_end = is_last_task ? pivot_start : sampled_data[end];
        for(size_t j=0; j<cur_task.size(); ++j){
