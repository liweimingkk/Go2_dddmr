#!/usr/bin/env python3
"""Versioned JSON I/O and pure policies for SCAN multi-point missions."""
...
class MissionValidationError(ValueError):
    """Raised when a mission or initial-pose document is unsafe or malformed."""
...
class ArrivalWindow:
    """Require a complete pose and stopped command for one continuous window."""

    def __init__(self, stable_sec: float) -> None:
        if not _finite_number(stable_sec) or stable_sec <= 0.0:
            raise MissionValidationError("arrival stable_sec must be finite and positive")
    ...
    def update(self, now_sec: float, pose_complete: bool, command_stopped: bool) -> bool:
        if not _finite_number(now_sec):
            self.reset()
            return False
        if not pose_complete or not command_stopped:
            self.reset()
            return False
...
def quaternion_from_yaw(yaw: float) -> Tuple[float, float, float, float]:
    ...
    return (0.0, 0.0, math.sin(normalized / 2.0), math.cos(normalized / 2.0))
...
def load_mission(
    ...
    *,
    ...
    if not isinstance(initial_reference, str) or not initial_reference:
        raise MissionValidationError("initial_pose_file must be a non-empty relative path")
    ...
    for index, raw_waypoint in enumerate(raw_waypoints):
        waypoint = _mapping(raw_waypoint, f"waypoints[{index}]")
        ...
        dwell_sec = _number(waypoint.get("dwell_sec"), f"waypoints[{index}].dwell_sec")
        if dwell_sec < 0.0 or dwell_sec > MAX_DWELL_SEC:
            raise MissionValidationError(
                f"waypoints[{index}].dwell_sec must be within [0, {MAX_DWELL_SEC:g}]"
            )
        yaw = _number(waypoint.get("yaw"), f"waypoints[{index}].yaw")
        ...
        waypoints.append(
            Waypoint(
                ...
                x=_number(waypoint.get("x"), f"waypoints[{index}].x"),
                y=_number(waypoint.get("y"), f"waypoints[{index}].y"),
                z=_number(waypoint.get("z"), f"waypoints[{index}].z"),
...
def initial_pose_document(
    ...
    # Validate the exact serialized shape before returning it.
    with tempfile.TemporaryDirectory() as directory:
        candidate = Path(directory) / "initial_pose.json"
        atomic_write_json(candidate, document)
        load_initial_pose(candidate)
    return document
        ...
        "waypoints": [
            {
                ...
                "y": waypoint.y,
                "z": waypoint.z,
                "yaw": normalize_angle(waypoint.yaw),
...
def _main() -> int:
    ...
    validate_parser.add_argument("--mission", required=True)
    validate_parser.add_argument("--root")
    id_parser = subparsers.add_parser("mission-id")
    id_parser.add_argument("--mission", required=True)
    id_parser.add_argument("--root")
    ...
    if arguments.command == "mission-id":
        ...
        print(f"VALID mission_id={mission.mission_id} waypoints={len(mission.waypoints)}")
    return 0
