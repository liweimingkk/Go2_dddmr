...
LegoLoamVisualization::LegoLoamVisualization(std::string name) : Node(name)
{
  ...
  ds_patched_ground_.setLeafSize(0.1, 0.5, 0.1); //we are in camera frame, z pointing to moving direction, y pointing to sky
...
void LegoLoamVisualization::groundEdgeDetectionThread() {
  ...
  ds_patched_ground_.setInputCloud(patched_ground);
  ds_patched_ground_.filter(*patched_ground);
