...
class Go2SportCmdVelAdapter(Node):
    def __init__(self) -> None:
        super().__init__("go2_sport_cmd_vel_adapter")
        ...
        self.enable_sport_output = bool(
            self.declare_parameter("enable_sport_output", False).value
        )
        self.allow_real_request_topic = bool(
            self.declare_parameter("allow_real_request_topic", False).value
        )
        self.stop_on_stale = bool(self.declare_parameter("stop_on_stale", True).value)
        ...
        self.request_id_base = int(
            self.declare_parameter("request_id_base", 0).value
        )
        ...
        self.request_seq = self.request_id_base
        self.request_msg_cls: Optional[Any] = None
        self.request_pub = None
        self.yaw_arc_started_time = None
        self.yaw_arc_fault_latched = False
        self.shutting_down = False

        self.publisher_enabled = self.compute_publisher_enabled()
        if self.publisher_enabled:
            self.configure_request_publisher()
    ...
    def compute_publisher_enabled(self) -> bool:
        if not self.enable_sport_output:
            return False
        if self.request_topic == REAL_REQUEST_TOPIC and not self.allow_real_request_topic:
            return False
        return True

    def configure_request_publisher(self) -> None:
        try:
            from unitree_api.msg import Request
        except ImportError as exc:
            raise RuntimeError(
                "unitree_api.msg.Request is required when sport output is enabled; "
                "source /home/lin/go2_workspace/unitree_ros2/setup.sh first"
            ) from exc

        self.request_msg_cls = Request
        self.request_pub = self.create_publisher(Request, self.request_topic, 10)

    def log_startup_state(self) -> None:
        if self.publisher_enabled:
            if self.request_topic == REAL_REQUEST_TOPIC:
                self.get_logger().warn(
                    "LIVE SPORT OUTPUT ENABLED: %s -> %s. This can move the Go2."
                    % (self.cmd_vel_topic, self.request_topic)
                )
            else:
                self.get_logger().warn(
                    "TYPED SPORT PREVIEW ENABLED: %s -> %s. This is not the real "
                    "Go2 sport request topic."
                    % (self.cmd_vel_topic, self.request_topic)
                )
        elif self.enable_sport_output and self.request_topic == REAL_REQUEST_TOPIC:
            self.get_logger().error(
                "SPORT OUTPUT BLOCKED: enable_sport_output=true but "
                "allow_real_request_topic=false for %s" % self.request_topic
            )
        else:
            self.get_logger().warn(
                "SPORT OUTPUT DISABLED: subscribing to %s; no %s publisher is created."
                % (self.cmd_vel_topic, self.request_topic)
            )

        self.get_logger().info(
            "limits: max_x=%.3f max_y=%.3f max_yaw=%.3f request_id_base=%d axis_mode=%s "
            "publish_rate_hz=%.3f cmd_timeout_sec=%.3f zero_epsilon=%.4f "
            "stop_keepalive_hz=%.3f stop_on_stale=%s"
            % (
                self.max_x,
                self.max_y,
                self.max_yaw,
    ...
    def publish_timer_cb(self) -> None:
        ...
        if self.stop_on_stale and age > self.cmd_timeout_sec:
        ...
        if self.publisher_enabled:
            self.publish_request(MOVE_API_ID, parameter)
            if self.should_log(now):
                self.get_logger().info(
                    "published %s: api_id=%d parameter=%s decision=%s "
                    "original_sport=%s transformed_sport=%s shim=%s"
                    % (
                        self.request_topic,
                        MOVE_API_ID,
                        parameter,
        ...
        if self.should_log(now):
            self.get_logger().info(
                "GATED would publish %s: api_id=%d parameter=%s decision=%s "
                "original_sport=%s transformed_sport=%s shim=%s"
                % (
                    self.request_topic,
                    MOVE_API_ID,
    ...
    def handle_stop(self, reason: str, now) -> None:
        if self.stop_sent_or_logged and not self.stop_keepalive_due(now):
            return

        if self.publisher_enabled:
            self.publish_request(STOP_MOVE_API_ID, "")
            self.get_logger().warn(
                "%s: published %s api_id=%d StopMove"
                % (reason, self.request_topic, STOP_MOVE_API_ID)
            )
        elif self.should_log(now) or not self.stop_sent_or_logged:
            self.get_logger().warn(
                "%s: GATED would publish %s api_id=%d StopMove"
                % (reason, self.request_topic, STOP_MOVE_API_ID)
            )
            self.last_log_time = now
    ...
    def publish_shutdown_stop(self, count: int = 3) -> None:
        ...
        for index in range(count):
            self.publish_request(STOP_MOVE_API_ID, "")
            self.get_logger().warn(
                "adapter shutdown: published %s api_id=%d StopMove (%d/%d)"
                % (self.request_topic, STOP_MOVE_API_ID, index + 1, count)
            )
            time.sleep(0.05)
...
def main() -> None:
    rclpy.init()
    node = Go2SportCmdVelAdapter()

    def raise_keyboard_interrupt(signum, _frame) -> None:
        node.get_logger().warn("received signal %d, shutting down" % signum)
    ...
    try:
        ...
        try:
            ...
            node.get_logger().error("failed to publish shutdown StopMove: %s" % exc)
        node.destroy_node()
        try:
            if rclpy.ok():
                rclpy.shutdown()
